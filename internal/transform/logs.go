...
func (c *Context) transformLogs(rm *logsv1.ResourceLogs) {
	...
	for _, sm := range rm.ScopeLogs {
		...
		for _, r := range sm.LogRecords {
			...
			if r.SeverityText != "" {
				c.label(
					labels.NewBytes(v1.PREFIX_LOGS_LEVEL).Value(r.SeverityText),
				)
			}
			c.attributes(v1.PREFIX_LOGS_ATTRIBUTES, r.Attributes)
			if r.SpanId != nil {
				c.label(
					labels.NewBytes(v1.PREFIX_LOGS_SPAN_ID).ValueBytes(r.SpanId),
				)
			}
			if r.TraceId != nil {
				c.label(
					labels.NewBytes(v1.PREFIX_LOGS_TRACE_ID).ValueBytes(r.TraceId),
				)
			}
		}
	}
