...
func (c *Context) transformTrace(rm *tracev1.ResourceSpans) {
	if rm.SchemaUrl != "" {
		c.label(
			labels.NewBytes(v1.SampleKind_TRACES, v1.PREFIX_RESOURCE_SCHEMA).Value(rm.SchemaUrl),
		)
	}
	if rm.Resource != nil {
		c.attributes(v1.SampleKind_TRACES, v1.PREFIX_RESOURCE_ATTRIBUTES, rm.Resource.Attributes)
	}
	for _, sm := range rm.ScopeSpans {
		if sm.SchemaUrl != "" {
			c.label(
				labels.NewBytes(v1.SampleKind_TRACES, v1.PREFIX_SCOPE_SCHEMA).Value(sm.SchemaUrl),
			)
		}
		if sc := sm.Scope; sc != nil {
			if sc.Name != "" {
				c.label(
					labels.NewBytes(v1.SampleKind_TRACES, v1.PREFIX_SCOPE_NAME).Value(sc.Name),
				)
			}
			if sc.Version != "" {
				c.label(
					labels.NewBytes(v1.SampleKind_TRACES, v1.PREFIX_SCOPE_VERSION).Value(sc.Version),
				)
			}
			c.attributes(v1.SampleKind_TRACES, v1.PREFIX_SCOPE_ATTRIBUTES, sc.Attributes)
		}
