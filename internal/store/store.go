...
func (s *Storage) Save(data *v1.Data) error {
	...
	err = ctx.Labels.Iter(func(lbl *labels.Label) error {
		return s.saveLabel(txn, txnData, lbl.Encode(), id)
	})
	if err != nil {
		return err
	}
...
func (s *Storage) saveLabel(txn *badger.Txn, ctx *Arena, key []byte, sampleID uint64) error {
	...
	if err != nil {
		// die , for sure bitmaps is the heart of rq.
		logger.Fail("BUG: failed writing label sample", "err", err)
	}
	b.RUnlock() // hot path manual unlock
